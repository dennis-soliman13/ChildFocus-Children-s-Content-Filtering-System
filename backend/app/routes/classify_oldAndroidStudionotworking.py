...
def _save_result(result: dict):
    """Save classification result + segments to DB."""
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.execute("""
...
def _map_label(score: float) -> str:
    """Map score to OIR label per thesis thresholds."""
...
def _action(label: str) -> str:
    """Map OIR label to system action per thesis."""
    return {
...
def _log_cache_hit(cached: dict, route: str, runtime: float):
    """Print classification summary for cache hits."""
...
def classify_full():
    ...
    Fuses with NB score → final Score_final → OIR label.
    ...
    # ── Cache check: only skip if already has heuristic_score (full analysis) ──
    cached = _get_cached(video_id)
    ...
    # ── Phase 3: Full heuristic analysis (downloads video) ────────────────────
    ...
    # ── Hybrid fusion: Score_final = α*NB + (1-α)*H ───────────────────────────
    ...
    result = {
        **heuristic_result,
        ...
        "nb_confidence":   nb_result.confidence if nb_result else 0.0,
        "heuristic_score": score_h,
        "final_score":     final_score,
        "label":           label,
        "oir_label":       label,           # alias used by Android
        "action":          action,
        "cached":          False,
...
def health():
    """System health check — model status, DB connectivity, cached video count."""
